...
namespace KP_Staff
{
    ...
    public partial class ChallengerPage : Page
    {
        ...
        {
            System.Diagnostics.Process.Start("https://vk.com/id504718041");
        }
        ...
        {
            var queryEmployee = from chal in workersEntities.Challenger orderby chal.surname select chal;
        ...
        private void cmbPayee_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            var res = from k in workersEntities.Position
                      select k;
        ...
        {
            Challenger challenger = workersEntities.Challenger.Create();
        ...
        private void DeleteCommandBinding_Executed(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            if (challenger != null)
            {
                workersEntities.Challenger.Remove(challenger);
        ...
        private void ButtonFindSurname_Click(object sender, RoutedEventArgs e)
        {
            ...
            var query = from chal in workersEntities.Challenger
            ...
            if (ListChallenger.Count > 0)
            {
                ...
                MessageBox.Show("Абитуриенты с ФИО \n" + surname + "\n не найдены", "Предупреждение",
        ...
        private void ButtonFindEdu_Click(object sender, RoutedEventArgs e)
        {
            ...
            var query = from chall in workersEntities.Challenger
            ...
            if (ListChallenger.Count > 0)
            {
                ...
                MessageBox.Show("Абитуриенты с образованием \n" + education + "\n не найдены", "Предупреждение",
                    MessageBoxButton.OK, MessageBoxImage.Warning);
        }
        ...
        private void ButtonFindBirth_Click(object sender, RoutedEventArgs e)
        {
            ...
            var query = from ch in workersEntities.Challenger
            ...
            if (ListChallenger.Count > 0)
            {
                ...
                MessageBox.Show("Абитуриенты с датой рождения от \n" + birth + "\n не найдены", "Предупреждение",
                    MessageBoxButton.OK, MessageBoxImage.Warning);
        }
        ...
        private void ButtonFindPosition_Click(object sender, RoutedEventArgs e)
        {
            ...
            var query = from s in workersEntities.Position
                        where s.C_name.Contains(position)
                        select s;
            foreach (Position p in query)
            {
                id = p.id_p;
                var queryChall = from chall in workersEntities.Challenger
                                 where chall.id_p == id
                                 select chall;
            ...
            if (ListChallenger.Count > 0)
            {
                ...
                MessageBox.Show("Абитуриенты с направлением \n" + position + "\n не найден", "Предупреждение",
                    MessageBoxButton.OK,
        ...
        private void btnFindAll_Click(object sender, RoutedEventArgs e)
        {
            ...
            var queryPos = from s in workersEntities.Position
                           where s.C_name.Contains(position)
                           select s;
            var queryChall = from chall in workersEntities.Challenger select chall;
            foreach (Position p in queryPos)
            {
                id2 = p.id_p;

                queryChall = from chall in workersEntities.Challenger
            ...
            if (ListChallenger.Count > 0)
            {
                ...
                MessageBox.Show("Абитуриенты по данным параметрам не найдены", "Предупреждение",
                    MessageBoxButton.OK,
                    MessageBoxImage.Warning);
        }
        ...
        {
            string postname = "";
            Staff staff = workersEntities.Staff.Create();
            Challenger challenger = DataGrid.SelectedItem as Challenger;
            ...
            try
            {
                ...
                isDirty = false;
                MessageBox.Show("Абитуриенту отправлено письмо о результате подачи заявления!");
            }
            catch (DataServiceRequestException ex)
            {
                throw new ApplicationException("Ошибка добавления данных" + ex.ToString());
        ...
        private void MailToChallenger(string email, string post)
        {
            ...
            MailAddress from = new MailAddress("menshikowa2004@yandex.ru", "Anna M.");
            // кому отправляем
            MailAddress to = new MailAddress(email);
            // создаем объект сообщения
            MailMessage m = new MailMessage(from, to);
            // тема письма
            m.Subject = "Результат подачи заявления";
            // текст письма
            m.Body = "<h2>Поздравляем! Вы приняты к нам " + post + "</h2>";
            // письмо представляет код html
            m.IsBodyHtml = true;
            // адрес smtp-сервера и порт, с которого будем отправлять письмо
            SmtpClient smtp = new SmtpClient("smtp.yandex.ru", 587);
            // логин и пароль
            smtp.Credentials = new NetworkCredential("menshikowa2004@yandex.ru", "zpfkozibnxkbhbpa");
            smtp.EnableSsl = true;
            smtp.Send(m);
            Console.Read();
        }
